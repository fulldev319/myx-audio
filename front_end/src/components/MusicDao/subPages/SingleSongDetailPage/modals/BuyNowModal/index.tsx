...
import { getMarketFee, updateOwnerSongOffer } from 'shared/services/API/PodAPI';
...
const BuyNowModal = ({ open, onClose, songDetailData, refresh }) => {
  ...
  const handleConfirm = async () => {
    ...
    const response = await web3APIHandler.openSalesManager.approvePurchase(
      ...
      {
        collection: songDetailData.nftAddress,
        tokenId,
    ...
    if (response.success) {
      ...
      //   PaymentToken: songDetailData?.sellingOffer.PaymentToken,
      //   Beneficiary: account?.toLowerCase(),
      //   from: songDetailData?.sellingOffer.Beneficiary,
      //   to: account?.toLowerCase(),
      //   hash: response.hash || '',
      //   isERC721: songDetailData.type !== 'ERC1155'
      // });
      onClose();
      refresh();
      //   handleRefresh();
