...
export class UniswapManager {
  ...
  async getUniswapPrices(
    ...
    try {
      const chainId: string = _chainId.includes('x')
      ...
      const selectedChainName =
        chainId === '3' ? 'ropsten' : chainId === '4' ? 'rinkeby' : 'mainnet';
      ...
      return {
        price: parseFloat(route.midPrice.toSignificant(_significant)),
        inverted: parseFloat(
          route.midPrice.invert().toSignificant(_significant)
        )
      };
    } catch (error) {
      console.log('getUniswapPrices', error);
      return { price: 0.0, inverted: 0.0 };
    }
  }
