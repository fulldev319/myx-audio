...
import arrow from 'assets/icons/swap_arrow.webp';
import spinner from 'assets/icons/spinner_blue.webp';
...
const Bridge = () => {
  ...
  const [isAllowanceRequired, setIsAllowanceRequired] =
    ...
    ws.onopen = () => {
      ...
      ws.onmessage = (msg: any) => {
        ...
        if (txBacklogRef.current) {
          if (
            output.action === Action.WITHDRAW_ERC20 ||
            output.action === Action.WITHDRAW_ETH
          ) {
            setTxBacklog(
              txBacklogRef.current.filter(
                (elem) => elem.random !== output.random
              )
            );
          } else {
            setTxBacklog(
              txBacklogRef.current.filter(
                (elem) => elem.txHash !== output.txHash
              )
            );
          }
        }
  ...
  /**
   * @notice  Execute a token transfer between Ethereum User's account & MUSIC User's account
   *          - Swap: Convert Ethereum into Fabric (MUSIC)
   *          - Withdraw: Convert Fabric (MUSIC) into Ethereum
   */
  ...
  //     if (isEthToFabric) {
  //       // Swap: Transfer amount from User's Wallet to MUSIC smart contract in Ethereum
  //       if (tokenName === 'ETH') {
  ...
  //       }
  //     } else {
  //       // Withdraw: Transfer amount from MUSIC smart contract to User's wallet in Ethereum
  //       if (tokenName === 'ETH') {
  ...
  /**
   * @notice  Swaps ethers or ERC20 tokens from Ethereum to MUSIC User's account
  ...
  //           setStatusBase({
  //             msg: 'Error while swaping with MUSIC blockchain',
  //             key: Math.random(),
  //             variant: 'error',
  //           });
  //         });
  ...
  /**
   * @notice  Withdraws ethers or ERC20 tokens from MUSIC to Ethereum User's account
   * @param   action: type of action to be performed
   * @param   fromAccount: origin account, normally User address in Metamask
   * @param   tokenName: name of ERC20 token (or ETH if transferring ether)
   * @param   chainId: Ethereum blockchain identifier (1: Mainnet, 3: Ropsten)
  ...
  return (
    <div className={'bridge_container'}>
      <div className={'box_bridge'}>
        ...
        <div className={'box_bridge_item'}>
          <div>
            <div className={'bridge_text'}>
              {' '}
              From {isEthToFabric ? 'ETHEREUM' : 'MUSIC'}
            </div>
            <InputWithLabelAndTooltip
              ...
              inputValue={amount}
              placeHolder="0.00"
              onInputValueChange={handleAmountChange}
              required
            />
          </div>
          <div>
        ...
        <button
          className={'arrow_button cursor-pointer'}
          onClick={handleIsEthToFabric}
        >
        ...
        <div className={'box_bridge_item'}>
          <div>
            <div className={'bridge_text'}>
              {' '}
              To {isEthToFabric ? 'MUSIC' : 'ETHEREUM'}
            </div>
            <InputWithLabelAndTooltip
              overriedClasses={'bridge_input'}
              disabled={true}
        ...
        {!isAllowanceRequired ||
        (isAllowanceRequired && transferCode < 3) ||
        !isEthToFabric ? (
          <button
