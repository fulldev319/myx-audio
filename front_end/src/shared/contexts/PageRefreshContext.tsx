...
const PageRefreshContext = React.createContext<PageRefreshContextType | null>(
  null
);

export const PageRefreshContextProvider: React.FC = ({ children }) => {
  const [requireMediaPageReload, setRequireMediaPageReload] =
    React.useState<boolean>(false);
  const [requireMarketPlacePageReload, setRequireMarketPlacePageReload] =
    React.useState<boolean>(false);
  ...
  return (
    <PageRefreshContext.Provider value={context}>
      {children}
...
export const usePageRefreshContext = () => {
  ...
  if (!context) {
    throw new Error(
      'usePageRefreshContext hook must be used inside PageRefreshContextProvider'
    );
  }
  return context;
};
