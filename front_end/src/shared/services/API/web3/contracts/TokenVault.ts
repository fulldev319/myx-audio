...
const tokenVault = (network: string) => {
  const updateReservePrice = async (
    ...
    try {
      ...
      const gas = await contract.methods
        .updateUserPrice(
          await web3.utils.toWei(payload.price.toString(), listTokenWeiUnit)
        )
      ...
      let error;
      if (e.toString().includes('not an update'))
        error = 'Not updating the price, same value as last price';
  ...
  const getReservePrice = async (
    ...
    try {
      ...
      return {
        success: true,
        data: await web3.utils.fromWei(resp, listTokenWeiUnit)
      };
    } catch (e) {
      console.log(e);
