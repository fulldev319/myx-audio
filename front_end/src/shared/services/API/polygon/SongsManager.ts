...
export async function registerSong(web3: Web3, account: string, song: any) {
  return new Promise(async (resolve) => {
    try {
      ...
      const artists =
        (song.artists &&
          song.artists.map((item) => web3.utils.keccak256(item.name))) ||
        [];

      const approveRes: any = await PolygonAPI.Music.approve(
        web3,
        account,
        'TRAX', //WIP: test case
        PolygonConfig.CONTRACT_ADDRESSES.SONGS_MANAGER
      ...
      const gas = await contract.methods
        .registerSong('test', genre, artists)
        .estimateGas({
          from: account
        });
      ...
      const {
        ...
        artistsPool,
        stakePool
      } = response.events.RegisterSong.returnValues;
