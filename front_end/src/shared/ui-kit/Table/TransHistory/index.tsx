...
} from 'shared/ui-kit/Select/TokenSelect';
import {
  CustomTable,
  CustomTableCellInfo,
  ...
  const getNoFilterTableData = () => {
    ...
    filteredtransactions()
      ...
      .map((transaction) => {
            ...
            <img
              src={
                transaction.Token && isCrypto(transaction.Token)
                  ? require(`assets/tokenImages/${transaction.Token}.webp`)
        ...
        row.push({
          cell: transaction.Chain ?? 'MUSIC',
          cellAlign: 'center'
        });
        row.push({
          cell: (
            <Moment format="ddd, DD MMM-h:mm A">
        ...
        row.push({
          cell: transaction.Id && (
            <a
              ...
              href={'https://musicscan.io/tx/' + transaction.Id}
            >
              <svg
                width="18"
                height="18"
    ...
    return transactions && transactions.length > 0
      ? transactions.filter((tx: any) => {
          ...
          return (
            ...
            (selectedDate &&
              new Date(tx.Date * 1000).getFullYear() >=
  ...
  return (
    <Box mt={'60px'}>
      <Box display="flex" alignItems="center" justifyContent="space-between">
        <div className={classes.headerbig}>
          {filter ? 'Recent transactions' : `Transactions`}
        </div>
        {filter ? (
    ...
    <div className={classes.pagination}>
      ...
      <Box mr="16px">
        <button
          ...
          disabled={page - 1 === -1}
        >
          <img src={require('assets/icons/arrow.webp')} alt="back" />
        </button>
        <button
          onClick={(e) => onChangePage(e, page + 1)}
