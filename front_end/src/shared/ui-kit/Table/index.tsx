...
const useStyles = makeStyles(() => ({
  ...
  tableRewards: {
    boxShadow: '0px 6px 13px rgba(91, 66, 121, 0.04)',
    borderRadius: 12,
    '& .MuiTableCell-head': {
      fontSize: FontSize.M,
      color: Color.Black,
  ...
  return (
    <TableContainer
      className={
        theme && theme === 'dark'
          ...
          ? classes.tableTransactionNoShadow
          : theme === 'artist'
          ? classes.tableArtist
          : theme === 'vote art'
          ? classes.tableVote
    ...
    >
      <Table>
                ...
                {header.sortable && (
                  <span style={{ marginLeft: '8px' }}>
                    <ArrowIcon
                      style={{
                        transform: `rotate(${
                          sorted[header.headerName] ? 0 : 180
                        }deg)`
                      }}
                    />
                  </span>
        ...
        <TableBody
          style={{
            background: variant === Variant.Tertiary ? 'white' : 'inherit'
          }}
        >
          {rows?.length > 0 ? (
            rows?.map((row, i) => (
              <TableRow
                ...
                style={
                  theme === 'transCards'
                    ? { background: getBackground(i + 1) }
                    : {}
                }
              >
                {row?.map((cellData, index) => (
                  <TableCell
                    align={cellData.cellAlign || 'inherit'}
                    key={cellData.cell + '-' + index}
                    style={
                      theme === 'transaction' && cellData.odd
