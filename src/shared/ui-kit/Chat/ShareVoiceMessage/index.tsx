...
const useStyles = makeStyles<Theme, StyleProps>((theme) => ({
  container: {
    background: ({ type }) =>
      type === 'pix'
        ? '#DDFF57'
        : 'linear-gradient(225.12deg, #F2A07E -33.5%, #FF5954 71.42%)',
    boxShadow: ({ type }) =>
      type === 'pix'
        ? '0px 4px 8px #9EACF2'
...
const ShareVoiceMessage: React.FC<{
  theme: ShareVoiceTheme;
  handleShare?: any;
}> = ({ theme = 'pix', handleShare = null }) => {
  ...
  const handleShareAudio = (file: any) => {
    ...
    axios
      .post(
        `${ServerURL()}/chat/shareAudio/${userSelector.id}`,
        formData,
        config
      )
      .then((response) => {
      ...
      >
        <Box maxWidth="90%">
          ...
          <Box className={classes.header2}>
            Share a voice memo with your followers.
          </Box>
        </Box>
