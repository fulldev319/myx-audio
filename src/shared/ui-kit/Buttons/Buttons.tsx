...
export default function Buttons(props) {
  ...
  const showAccount = () => {
    if (user.ethAccount && user.ethBalance && user.web3 && netId) {
      ...
      const account =
        user.ethAccount.slice(0, 4) + '..' + user.ethAccount.slice(-4);
      ...
      <ConnectButton onClick={handleOpenConnect}>
        {user.ethAccount === '' ? (
          ...
          <span className="connected">
            ...
            <img
              src={require('assets/icons/ethereum-brands.svg')}
              alt={'ethereum-brands'}
            />{' '}
            {showAccount()}
          </span>
          ...
          <span className="connected">
            ...
            <img
              className="iconn"
              src={require('assets/tokenImages/BNB.webp')}
              alt={'BNB'}
            />
            {showAccount()}
          </span>
