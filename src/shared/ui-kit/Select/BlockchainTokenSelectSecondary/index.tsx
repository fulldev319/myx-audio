...
export const BlockchainTokenSelectSecondary = ({
  ...
  const blockchain = BlockchainNets.find(
    (blockChainNet) => blockChainNet['name'] === communityToken?.Network
  );
  return (
    <div>
      <FormControl variant="outlined" style={{ width: '100%' }}>
        <StyledSelectDao
          className={
            theme === 'music dao' ? classes.selectMusicDao : classes.select
          }
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
              {communityToken.Network && blockchain && (
                <img
                  src={require(`assets/${
                    blockchain.image ??
                    'tokenImages/' + communityToken.Network + '.webp'
                  }`)}
