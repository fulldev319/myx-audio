...
const txnTypeMap = {
  MUSIC_credit_creation: 'Creation',
  MUSIC_credit_deposit: 'Lent',
  MUSIC_credit_borrowing: 'Borrowed',
  MUSIC_credit_collateral: 'Collateral',
  MUSIC_credit_interest: 'Interest',
  MUSIC_credit_withdraw: 'Withdraw',
  MUSIC_credit_modified: 'Modified',
  MUSIC_risk_taking: 'Assume Risk',
...
export default function TradingHistory(props) {
  ...
  useEffect(() => {
    ...
    headers.push({ headerName: 'MUSICSCAN' });
  ...
  useEffect(() => {
    ...
    if (datedList && datedList.length) {
      data = datedList.map((row) => {
        ...
        oneRow.push({
          cell: (
            <div style={{ color: 'rgba(101, 110, 126, 0.7)' }}>{row.time}</div>
          )
        });
        ...
        oneRow.push({
          cell: row.id ? (
            <a
              ...
              href={'https://musicscan.io/tx/' + row.id}
            >
