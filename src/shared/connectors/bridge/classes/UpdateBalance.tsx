...
import MUSICPodERC721TokenContractJson from 'shared/contracts/ABI_V5/MUSICPodERC721Token.json';
...
export class UpdateBalance {
  ...
  async retrieveERC20Balance(
    ...
    try {
      const arrayRegisteredToken =
        await this.bridgeTokenManager.getBridgeRegisteredErc20TokenFromMetamask(
          String(chainId),
          web3
        );
      console.log(
        'retrieveERC20Balance arrayRegisteredToken',
        arrayRegisteredToken
      );

      const tempArrayOfTokens: any[] = [
        {
          id: 0,
          name: SupportedNetworkCoin[chainId].name,
          symbol: SupportedNetworkCoin[chainId].symbol,
          amount: ethBalance,
      ...
      arrayRegisteredToken.forEach((element, index) => {
        tempArrayOfTokens.push({
          ...
          amount: 0.0,
          address: element.deployedAddress
        });
      });
      ...
      for (let index = 0; index < tempArrayOfTokens.length; index++) {
        if (
          tempArrayOfTokens[index].address !==
            '0x0000000000000000000000000000000000000000' &&
          tempArrayOfTokens[index].symbol !==
            SupportedNetworkCoin[chainId].symbol
        ) {
          const tokenContract = new web3.eth.Contract(
            IERC20ContractJson.abi,
            tempArrayOfTokens[index].address
          );
          try {
            ...
            const floatBalance = Number(
              web3.utils.fromWei(userTokenBalance, 'ether')
            );
            ethBalances.push({
              id: tempArrayOfTokens[index].id,
              name: tempArrayOfTokens[index].name,
              symbol: tempArrayOfTokens[index].symbol,
              amount: floatBalance,
              address: tempArrayOfTokens[index].address
            });
          } catch (e) {
  ...
  async retrieveERC721Balance(
    ...
    try {
      const arrayRegisteredToken =
        await this.bridgeTokenManager.getBridgeRegisteredErc721TokenFromMetamask(
          String(chainId),
          web3
        );
      console.log(
        'retrieveERC721Balance arrayRegisteredToken',
        arrayRegisteredToken
      );
      ...
      for (let index = 0; index < tempArrayOfTokens.length; index++) {
        if (
          tempArrayOfTokens[index].address !==
            '0x0000000000000000000000000000000000000000' &&
          tempArrayOfTokens[index].address !== '0x'
        ) {
          const tokenContract = new web3.eth.Contract(
            MUSICPodERC721TokenContractJson.abi,
            tempArrayOfTokens[index].address
          );
          const userTokenBalance = await tokenContract.methods
            .balanceOf(fromAddress)
          ...
          for (let index = 0; index < userTokenBalance; index++) {
            const tokenId = await tokenContract.methods
              .tokenOfOwnerByIndex(fromAddress, index)
  ...
  async retrieveEthereumBalance(addr: string, web3: any) {
    ...
    if (chainId > 0) {
      ...
      if (typeof addr !== 'undefined') {
        ...
        const balancesERC20 = await this.retrieveERC20Balance(
          addr,
          chainId,
          web3,
          balanceEther
        ...
        const balancesERC721 = await this.retrieveERC721Balance(
          addr,
          chainId,
          web3,
          balanceEther
        ...
        if (balancesERC20) {
          ...
          console.log(
            'retrieveEthereumBalance balancesERC20.length',
            balancesERC20.length
          );

          // Update all Ethereum balances in state
          if (balancesERC721) {
            this.dispatch(setEthBalance([...balancesERC20, ...balancesERC721]));
          } else {
            this.dispatch(setEthBalance(balancesERC20));
          }
        }
      }
    }
  ...
  removeAccount() {
    // User disconnects from the MUSIC web in Metamask
