...
let walletTypes = ['Music Wallet', 'Metamask', 'WalletConnect'];

let walletImgs = ['music_wallet', 'metamask', 'wallet_connect'];
...
const WalletInfoContainer = ({
  ...
  const { id, walletStatus, walletType } = walletInfo;

  if (id && id.length > 0 && walletType) {
    return (
      <div
        className={`wallet_info_container cursor-pointer ${
          isActive ? 'active_border' : 'disabled_border'
        }`}
      ...
      >
        ...
        <div className="wallet_code">
          {walletType > -1 ? (
            <img
              src={require(`assets/walletImages/${walletImgs[walletType]}.svg`)}
            />
          ) : null}
          <span className={`${isActive ? 'active' : 'disabled'}`}>{`${id.slice(
            0,
            5
          )}...${id.slice(id.length - 4, id.length)}`}</span>
        </div>
  ...
  return (
    <>
      ...
      <div className="extra_container">
        {!shouldShowAll && (
          <button className="see_all" onClick={() => setShouldShowAll(true)}>
            See All
          </button>
        )}
        <button onClick={handleAddWallet} className="add_wallet">
          <AddIcon
            style={{ width: '16px', marginRight: '8px', marginBottom: '4px' }}
          />
