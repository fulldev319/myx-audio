...
type UserActionContextType = {
  ...
  followUser: (userId: string, isTraxUser: boolean) => Promise<FollowResult>;
...
export const UserConnectionsContextProvider: React.FunctionComponent = ({
  children
}) => {
  ...
  const context = useMemo<UserActionContextType>(
    () => ({
      followers,
      followings,
      async followUser(userId: string, isTraxUser: boolean = false) {
        const result = await followUser(userId, false, null, isTraxUser);
      ...
      async unfollowUser(userId: string) {
        setFollowings((followings) =>
          followings.filter((entry) => entry.id !== userId)
        );
...
export const useUserConnections = () => {
  ...
  if (!context) {
    throw new Error(
      'useUserConnections hook must be used inside UserConnectionsContextProvider'
    );
  }
