...
const TokenConversionContext = createContext<TokenConversionContextType | null>(
  null
);

export const TokenConversionContextProvider: React.FunctionComponent = ({
  children
}) => {
    ...
    axios.get(`${URL()}/wallet/getCryptosRateAsMap`).then((res) => {
      ...
      if (resp.success) {
        ...
        const parsedTokens: Record<string, number> = Object.fromEntries(
          Object.entries(data).map(([token, rate]) => [
            token.toUpperCase(),
            parseFloat(rate)
          ])
        );
  ...
  return (
    <TokenConversionContext.Provider value={context}>
      {children}
