...
export const NotificationsContextProvider: React.FunctionComponent<
...
> = ({ socket, children }) => {
  ...
  useEffect(() => {
    if (currentUserId) {
      getNotifications().then((result) => {
        if (result.success) {
          const podNotifications = result.data.notifications.filter(
            (item) => !item.podType || item.podType === 'TRAX'
          );
  ...
  useEffect(() => {
    if (currentUserId && socket) {
      const incomingNotificationHandler = (
        incomingNotification: Notification
      ) => {
        if (
          !incomingNotification.podType ||
          incomingNotification.podType === 'TRAX'
        ) {
...
export const useNotifications = () => {
  ...
  if (!context) {
    throw new Error(
      'useNotifications hook must be used inside NotificationsContextProvider'
    );
  }
