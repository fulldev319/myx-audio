...
const vaultFactory = (network) => {
  const contractAddress =
    config[network].CONTRACT_ADDRESSES.ERC721_VAULT_FACTORY;
      ...
      try {
        ...
        const gas = await contract.methods
          .mint(
            ...
            payload.token,
            payload.id,
            await web3.utils.toWei(payload.supply.toString(), 'ether'),
            payload.asset,
            await web3.utils.toWei(
              payload.listPrice.toString(),
              listTokenWeiUnit
            ),
            payload.fee * 10
          )
        ...
        const erc20VaultTokenAddress =
          response?.events?.Mint?.returnValues?.vault;
