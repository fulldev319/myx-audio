...
const loan = (network) => {
  const contractAddress =
    config[network].CONTRACT_ADDRESSES.ERC721_COLLATRALISE_AUCTION;
  ...
  const getAuction = async (web3: Web3, payload: any): Promise<any> => {
    return new Promise(async (resolve) => {
      try {
        const contract = ContractInstance(web3, metadata.abi, contractAddress);
        contract.methods
          .getAuction(payload.tokenContractAddress, payload.tokenId)
          .call((err, result) => {
            if (err) {
              console.log(err);
              resolve(null);
  ...
  const getBidList = async (web3: Web3, payload: any): Promise<any> => {
    return new Promise(async (resolve) => {
      try {
        const contract = ContractInstance(web3, metadata.abi, contractAddress);
        contract.methods
          .getBidList(payload.tokenContractAddress, payload.tokenId)
          .call((err, result) => {
            if (err) {
              console.log(err);
              resolve(null);
