...
const music_metadata = require('shared/connectors/polygon/contracts/MUSIC.json');
  ...
  return new Promise(async (resolve) => {
    try {
      const contract = ContractInstance(
        web3,
        tokenList[token].abi,
        PolygonConfig.TOKEN_ADDRESSES[token]
      );
      const allowance = await contract.methods
        .allowance(account, address)
        .call();

      const decimals = await PolygonAPI.Music.decimals(web3, token);
