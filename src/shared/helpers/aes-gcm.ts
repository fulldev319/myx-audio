  ...
  return new Promise((resolve, reject) => {
    callOnStore(function (store) {
      ...
      request.onsuccess = async function () {
        if (request.result && request.result.aesKey)
          resolve({ aesKey: request.result.aesKey });
...
export function callOnStore(fn_) {
  ...
  open.onupgradeneeded = function () {
    var db = open.result;
    var store = db.createObjectStore('MusicStore', { keyPath: 'id' });
  };
  open.onsuccess = function () {
    // Start a new transaction
    var db = open.result;
    var tx = db.transaction('MusicStore', 'readwrite');
    var store = tx.objectStore('MusicStore');
    fn_(store);
...
export async function saveMusicKey(privateKey: Uint8Array) {
  let aesKey;
  try {
  ...
  localStorage.setItem(
    'MUSIC_WALLET_KEY',
    JSON.stringify({ buffer: cipherBuffered, iv: ivArrayTyped })
  );
}

export async function loadMusicKey(): Promise<Uint8Array> {
  const { aesKey } = await loadDecryptionKey();
  const { buffer, iv: ivArrayTyped } = JSON.parse(
    localStorage.getItem('MUSIC_WALLET_KEY') || ''
  );
