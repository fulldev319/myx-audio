...
export function formatNumber(number, token, numDecimals): string {
  let formattedNum: any = 0;
  if (number >= billion)
    formattedNum =
      parseFloat((number / billion).toFixed(numDecimals)).toString() +
      billionSymbol;
  else if (number >= million)
    formattedNum =
      parseFloat((number / million).toFixed(numDecimals)).toString() +
      millionSymbol;
  else
    formattedNum = parseFloat(Number(number).toFixed(numDecimals)).toString();
  ...
  else formattedNum = (currencyMap[token] ?? '') + formattedNum;
  return formattedNum;
}

// return a random color from the following palette
const collabCardColors = [
  '#ACDDDE',
  '#CAF1DE',
  '#E1F8DC',
  '#FEF8DD',
...
export function addUniqueIdToArray(arr: any[]): any[] {
  if (Array.isArray(arr)) {
    return arr.map((a) => {
      a._musicUniqueId = generateUniqueId();
      return a;
    });
  }
  ...
  if (Array.isArray(obj)) {
    content = obj.map((item, index) => {
      if (item && typeof item === 'string')
        ...
        return (
          <div style={{ paddingLeft: '10px' }}>{convertObjectToJsx(item)}</div>
        );
      }
      return <> </>;
    });
...
export function calculateTimeFromNow(date: number) {
  ...
  return weeksFromNow > 0
    ? `${weeksFromNow} week${weeksFromNow > 1 ? 's' : ''} ago`
    : daysFromNow > 0
    ? `${daysFromNow} day${daysFromNow > 1 ? 's' : ''} ago`
