...
import ListSongCard from 'components/MusicDao/components/Cards/ListSongCard';
...
import { singleSongEditionsPageStyles } from './index.styles';
...
export default function SingleSongEditionsPage() {
  const classes = singleSongEditionsPageStyles();
  ...
  const handleOpensea = () => {
    ...
    window.open(
      `https://${!isProd ? 'testnets.' : ''}opensea.io/assets/${
        !isProd ? 'mumbai' : 'matic'
      }/${songDetailData?.nftAddress || songDetailData?.albumId}/${
    ...
    <>
      <Box display="flex" alignItems="center">
        ...
        <Box className={classes.tag}>
          {songDetailData?.Mood}{' '}
          {songDetailData?.Mood
            ? MoodEmoji[Mood.findIndex((mood) => mood === songDetailData.Mood)]
            : ''}
        </Box>
      </Box>
      ...
      >
        {isListView ? (
          <>
            {(isLoadingEditions ? Array(4).fill({}) : editions).map(
              (song, index) => (
                <ListSongCard
                  data={song}
    ...
    <Box
      ...
      style={{
        backgroundImage: isMobile
          ? ''
          : `linear-gradient(180deg, rgba(238, 242, 246, 0) -35.9%, #EEF2F6 40.45%), url(${
              songImageIPFS ??
              require('assets/musicDAOImages/editions_back.webp')
            })`,
            ...
            <Box display="flex" justifyContent="space-between" mt={7}>
              <Box display="flex" flexDirection="column" width={450}>
                ...
                <div className={classes.ownerSection}>
                  <div className={classes.typo1}>Creator</div>
                  <Box
                    display="flex"
                    justifyContent="space-between"
                    alignItems="center"
              ...
              <Box flex={1} ml={5} style={{ width: 'calc(100% - 450px)' }}>
                <Box className={classes.typo5}>{songDetailData?.Title}</Box>
                <Box
                  display="flex"
                  pb={2}
                  mb={2}
          ...
          {isMobile && (
            <Box display="flex" flexDirection="column" mt={5}>
              <Box display="flex" flexDirection="column" mx={2}>
                ...
                <div className={classes.addressSection}>
                  <Box display="flex" flexDirection="column" width="90%" mr={1}>
                    <div
                      className={classes.typo1}
                    >{`Preview on ${songDetailData?.scanNetworkName}`}</div>
                    <div
...
export const OpenseaIcon = () => (
  <svg
    ...
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clip-path="url(#clip0_7431_101126)">
      <path
