...
export default function FullVote() {
  ...
  useEffect(() => {
    if (params?.id) {
      ...
      Axios.get(
        `${URL()}/musicDao/governance/polls/getDetails/${params.podId}/${
          params.id
        }`
      )
        .then((res) => {
  ...
  const votePoll = () => {
    if (params && params.id && params.podId) {
      const body = {
        pollId: params?.id,
        podId: params?.podId,
        ...
        .then((res) => {
          ...
          if (data.success) {
            setVote((prev) => ({
              ...
              votes: [
                ...(prev.votes || []),
                { userAddress: user.id, answer: body.answer }
              ]
            }));
          }
        })
