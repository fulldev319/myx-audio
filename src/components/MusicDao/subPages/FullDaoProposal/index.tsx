...
import {
  GreenLinearProgress,
  RedLinearProgress
} from 'components/MusicDao/components/LinearProgress';
...
export default function FullDaoProposal() {
  ...
  const [filterOptionsSelection, setFilterOptionsSelection] = useState<string>(
    filterOptions[0]
  );
  ...
  useEffect(() => {
    if (params?.id) {
      ...
      Axios.get(
        `${URL()}/musicDao/governance/proposals/getDetails/${params.id}`
      )
        .then((res) => {
          const data = res.data;
          if (data.success) {
        ...
        .then((res) => {
          ...
          if (data.success) {
            setProposal((prev) => ({
              ...prev,
              replies: [
                { userAddress: user.id, text: comment, created: Date.now() },
                ...prev.replies
              ]
            }));
            setComment('');
                ...
                <Box className={classes.flexBox}>
                  ...
                  <img
                    src={require(`assets/tokenImages/${
                      resImages.findIndex(
                        (image) => image === proposal.votingToken
                      ) > 0
            ...
            <Box mt={5} className={classes.proposalDetailBox} zIndex={1}>
              ...
              <div className={classes.voteSection}>
                ...
                <Box mt={3} className={classes.flexBox} width={1}>
                  <Box width={1} position="relative">
                    <GreenLinearProgress
                      ...
                      value={
                        Math.round(
                          ((proposal.votes?.filter((vote) => vote.answer)
                            .length || 0) /
                            proposal.totalVotes) *
                            100 *
                            100
                  ...
                  <Box className={classes.header2} ml={1} minWidth="50px">
                    {Math.round(
                      ((proposal.votes?.filter((vote) => vote.answer).length ||
                        0) /
                        proposal.totalVotes) *
                        100 *
                        100
                ...
                <Box className={classes.flexBox} mt={6} height="45px">
                  <Box
                    className={`${classes.blackBox} ${classes.flexBox}`}
                    width={1}
                    height={1}
                  >
                    <img
                  ...
                  <Box
                    className={`${classes.greyBox} ${classes.flexBox}`}
                    ml={2}
                    height={1}
                  >
                    <CustomPopup
                  ...
                  <Hidden only="xs">
                    <Box ml={4} width={1} height={1}>
                      <PrimaryButton
                        ...
                        isRounded
                        disabled={
                          proposal.endDate < new Date().getTime() ||
                          getMyAnswer()
                        }
                      >
                        Vote
                ...
                <Hidden smUp>
                  <Box className={classes.flexBox} mt={6} height="45px">
                    <PrimaryButton
                      ...
                      isRounded
                      disabled={
                        proposal.endDate < new Date().getTime() || getMyAnswer()
                      }
                    >
                      Vote
                    </PrimaryButton>
