...
import { Card, GreenText } from 'components/MusicDao/index.styles';

const arePropsEqual = (prevProps, currProps) => {
  return (
    prevProps.userProfile === currProps.userProfile &&
    prevProps.myStats === currProps.myStats
  );
};

const InfoStats = React.memo(
  ({ userProfile, myStats }: { userProfile: any; myStats: any }) => {
    return (
      <Card>
        <Box display="flex" alignItems="center" justifyContent="space-between">
          ...
          <Box fontSize="14px" alignItems="center">
            <img
              src={require('assets/icons/info_gray.webp')}
              alt="info"
              width="13.5px"
              height="13.5px"
        ...
        <Box mb={2} width="100%">
          <StyledDivider
            type="solid"
            color={Color.GrayInputPlaceholder}
            margin={2}
          />
        </Box>
        <Grid container wrap="wrap" spacing={4}>
          <Grid item xs={6} sm={4} md={3}>
            <StatInfo
              ...
              label={`🤝 Trust`}
              tooltipInfo={
                'A measure of financial trustworthiness, Trust Scores quantify transaction history data on individuals and Capsules within the system'
              }
            />
          </Grid>
          <Grid item xs={6} sm={4} md={3}>
            <StatInfo
              ...
              label={`⚔️ Endorsement`}
              tooltipInfo={
                'A measure of financial trustworthiness, Endorsement Scores quantify the endorsements (and their trust scores) of individuals and/or Capsules'
              }
            />
          </Grid>
          <Grid item xs={6} sm={4} md={3}>
            <StatInfo
              value={myStats ? myStats.myMediasCount : 0}
              label="📹 Media"
          ...
          <Grid item xs={6} sm={4} md={3}>
            <StatInfo
              ...
              label="💰 Creds"
              tooltipInfo={
                'Creds received in discussion, posts and other publications by other users.'
              }
            />
          </Grid>
          <Grid item xs={6} sm={4} md={3}>
            <StatInfo
              value={
                (myStats?.myFTPodsCount ?? 0) +
                  (myStats?.myDigitalPodsCount ?? 0) ?? 0
              }
              label="📈 Investments"
          ...
          <Grid item xs={6} sm={4} md={3}>
            <StatInfo
              value={myStats?.myDigitalPodsCounts ?? 0}
              label="💊 Capsule"
              tooltipInfo={'Capsule info'}
            />
          </Grid>
        </Grid>
      </Card>
