...
export function DiscussionPage(props) {
  ...
  const downloadFile = (val) => {
    saveAs(
      `${URL()}/podDiscussion/downloadFile/${props.podId}/${topicView.id}/${
        val.message
      }`,
      val.message
    );
  ...
  const onChangeMessageFile = (file: any) => {
    ...
    axios
      .post(
        `${URL()}/podDiscussion/addMessageFile/${props.podId}/${topicView.id}/${
          user.id
        }`,
        formData,
        config
  ...
  const onChangeMessageVideo = (file: any) => {
    ...
    axios
      .post(
        `${URL()}/podDiscussion/addMessageVideo/${props.podId}/${
          topicView.id
        }/${user.id}`,
        formData,
        config
  ...
  const onChangeMessagePhoto = (file: any) => {
    ...
    axios
      .post(
        `${URL()}/podDiscussion/addMessagePhoto/${props.podId}/${
          topicView.id
        }/${user.id}`,
        formData,
        config
  ...
  const onChangeMessageAudio = (file: any) => {
    ...
    axios
      .post(
        `${URL()}/podDiscussion/addMessageAudio/${props.podId}/${
          topicView.id
        }/${user.id}`,
        formData,
        config
  ...
  const handleFilesAttachment = (files) => {
    for (let i = 0; i < files.length; i++) {
      if (files[i].type.startsWith('image'))
        handleFiles({ 0: files[i], length: 1 });
      else if (files[i].type.startsWith('audio'))
        handleFilesAudio({ 0: files[i], length: 1 });
      else if (files[i].type.startsWith('video'))
        handleFilesVideo({ 0: files[i], length: 1 });
  ...
  const loadTopic = (isNew?: boolean): Promise<number> => {
    return new Promise((resolve, reject) => {
      ...
      axios
        .get(
          `${URL()}/podDiscussion/new/getMessages/${props.podId}/${
            topicView.id
          }/${isNew ? 0 : pageIndex}`
        )
        .then((response) => {
                ...
                >
                  {val.from !== user.id ? (
                    <Box
                      ...
                      style={{
                        backgroundImage: `url("${
                          processImage(val.user?.imageUrl) ?? getDefaultAvatar()
                        }")`,
                  ...
                  {val.type === 'text' ? (
                    ...
                    >
                      <Box
                        ...
                        onClick={() => {
                          setSelectedPhoto(
                            `${URL()}/podDiscussion/getMessagePhoto/${
                              props.podId
                            }/${topicView.id}/${val.message}`
                          );
                          handleOpenModalDiscordPhotoFullScreen();
                        }}
                        style={{
                          backgroundImage: `url(${URL()}/podDiscussion/getMessagePhoto/${
                            props.podId
                          }/${topicView.id}/${val.message})`,
                          backgroundPosition: 'center',
                          backgroundRepeat: 'no-repeat',
                    ...
                    >
                      <Box className="messageInfoVideo">
                        ...
                        <ReactPlayer
                          onClick={() => {
                            setSelectedVideo(
                              `${URL()}/podDiscussion/getMessageVideo/${
                                props.podId
                              }/${topicView.id}/${val.message}`
                            );
                            handleOpenModalDiscordVideoFullScreen();
                          }}
                          url={`${URL()}/podDiscussion/getMessageVideo/${
                            props.podId
                          }/${topicView.id}/${val.message}`}
                    ...
                    >
                      <Box className="messageInfoAudio">
                        <Waveform
                          url={`${URL()}/podDiscussion/getMessageAudio/${
                            props.podId
                          }/${topicView.id}/${val.message}`}
                          mine={false}
                          showTime={false}
                    ...
                    >
                      <Box className="item-file">
                        <Box className="item-file-name">
                          {val.message.originalname}
                        </Box>
                        <Box
              ...
              <form onSubmit={handleSubmit}>
                <TextareaAutosize
                  ...
                  onChange={(messageContent) =>
                    setChatContent(messageContent.target.value)
                  }
