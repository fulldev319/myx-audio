...
import CreateMultipleEditions from '../CreateMultipleEditions';
...
import { createMutipleEditionsPodStyles } from './index.styles';
...
export default function CreateMutipleEditionsPod({
  open,
  onClose,
  editMode,
  draft,
  ...
  const classes = createMutipleEditionsPodStyles();
  ...
  const Tabs = React.useMemo(() => {
    return type === ContentType.SongSingleEdition
      ? TabsSingleEdition
      : TabsMultiEdition;
  }, [type]);
  ...
  const uploadSong = async (value) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setSongData((prev) => ({ ...prev, metadataMedia, chunks }));
        setSong(value);
      }
  ...
  const validatePage = () => {
    ...
    // }
    if (type === ContentType.SongSingleEdition) {
      if (!songData.price) validationStatus[0] = false;
    } else {
      if (
    ...
    if (songData.editionType === 'Premium') {
      if (songData.editionAmounts.every((edition) => Number(edition) <= 0)) {
        ...
        if (res.filter((val) => !val).length > 0) validationStatus[0] = false;
      }
    }
    if (
    ...
    if (!songData.Title) {
      if (type === ContentType.SongSingleEdition) validationStatus[3] = false;
  ...
  const validate = () => {
    if (type === ContentType.SongSingleEdition) {
      ...
      showAlertMessage(`You need to set amount and price of editions.`, {
        variant: 'error'
      });
      return false;
    } else if (songData.editionType === 'Premium') {
      if (songData.editionAmounts.every((edition) => Number(edition) <= 0)) {
        showAlertMessage(`Please set amount of price of editions correctly`, {
          variant: 'error'
        });
        return false;
      } else {
    ...
    if (
      ...
      if (
        ...
        showAlertMessage(`Streaming Revenue Shares should be 100%.`, {
          variant: 'error'
        });
        return false;
      }
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div className={classes.workSpace}>
              <div className={classes.content}>
                ...
                {Tabs[page].marker === 'Price' ? (
                  <PriceTab songData={songData} setSongData={setSongData} />
              ...
              <div className={classes.buttons}>
                <SecondaryButton
                  ...
                  style={{
                    ...
                    width:
                      isMobile && Tabs[page].marker === 'Upload'
                        ? 'auto'
                        : isMobile
                        ? '40%'
                        : '200px'
