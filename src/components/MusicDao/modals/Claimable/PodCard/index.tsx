...
import ClaimPodModal from '../ClaimPod';
...
import { ReactComponent as MusicChainIcon } from 'assets/icons/musicChainIcon.svg';

const videoIcon = require('assets/mediaIcons/small/video.webp');
const audioIcon = require('assets/mediaIcons/small/audio.webp');
const blogSnapIcon = require('assets/mediaIcons/small/blog_snap.webp');
...
export default function PodCard({ variant = Variant.Primary, ...props }) {
  ...
  const handleOpenShareModal = () => {
    shareMediaToSocial(pod.id || pod.urlSlug, 'Capsule', props.type);
  };
      ...
      .then((response) => {
        if (response.data.success) {
          if (itemCopy.bookmarked) {
            ...
            if (itemCopy.Bookmarks) {
              itemCopy.Bookmarks = itemCopy.Bookmarks.filter(
                (item) => item.userId !== user.id
              );
            }
          }
  ...
  return (
    <Box className={styles.podCard}>
      <Box className={styles.podImageContent}>
        ...
        {pod.CreatorImageURL ? (
          <Box
            ...
            style={{
              backgroundImage: pod.CreatorImageURL
                ? `url(${pod.CreatorImageURL})`
                : 'none',
              backgroundRepeat: 'no-repeat',
              backgroundSize: 'cover',
              backgroundPosition: 'center',
        ...
        {props.type !== 'claim-music-dao' && (
          <Box className={styles.socialButtons}>
            <span className={styles.clickable} onClick={handleBookmark}>
              <img
                src={require(isBookmarked()
                  ? 'assets/musicIcons/bookmark-filled.svg'
                  : 'assets/musicIcons/bookmark.svg')}
                alt="bookmark"
              />
            </span>
            <span
            ...
            <span onClick={handleLikeCard} className={styles.clickable}>
              <img
                src={require(isLike()
                  ? 'assets/musicIcons/heart_filled.webp'
                  : 'assets/musicIcons/heart.webp')}
                alt={'heart'}
              />
            </span>
          </Box>
        ...
        {variant === Variant.Primary && (
          <Box className={`${styles.podStatus} ${styles.topRightBox}`}>
            {pod.Status}
          </Box>
        )}
      ...
      <Box className={styles.podInfo}>
              ...
              onClick={() => {
                if (!props.disableClick) {
                  if (props.isFractionalised) {
                    history.push(`/capsules/fractions/${pod.PodAddress}`);
                  } else if (
                    props.type === 'claim-music' ||
                    props.type === 'claim-video'
                  ) {
                    history.push(`/capsules/claim-music/${pod.id}`);
                  } else if (props.type === 'claim-music-dao') {
                    history.push(`/claimable-music/${pod.id}`);
                  } else if (props.type === 'Digital-NFT') {
                    history.push(`/capsules/MediaNFT/${pod.PodAddress}`);
                  } else {
                    if (props.type.includes('NFT')) {
                      history.push(`/capsules/MediaNFT/${pod.PodAddress}`);
                    } else {
                      history.push(`/capsules/FT/${pod.PodAddress}`);
                    }
                  }
                }
              }}
        ...
        {props.type === 'claim-music-dao' && (
          <Box className={classnames(styles.divider, 'secondary')} />
        )}
        <Box className={styles.podMainInfo}>
          {variant !== Variant.Tertiary && (
            <Box
              className={styles.reproductionContent}
              display="flex"
          ...
          {variant === Variant.Primary && (
            <Box className={styles.podMainInfoContent}>
              ...
              <Box>
                ...
                <p>
                  {pod.InvestorDividend
                    ? `${pod.InvestorDividend * 100}%`
                    : '0%'}
                </p>
              </Box>
            </Box>
              ...
              <Box className={styles.fundInfo}>
                <Box
                  className={classnames(
                    styles.fundValue,
                    props.type === 'claim-music-dao' && 'secondary'
                  )}
                >
