...
import { musicDaoGetEditionsOfSong } from 'shared/services/API';
...
export default function SingleSongDetailPage() {
  ...
  const [openSellingOfferModal, setOpenSellingOfferModal] =
    useState<boolean>(false);
  const [openCancelSaleModal, setOpenCancelSaleModal] =
    useState<boolean>(false);
  const [openRemoveOfferModal, setOpenRemoveOfferModal] =
    useState<boolean>(false);
  const [openAcceptOfferModal, setOpenAcceptOfferModal] =
    useState<boolean>(false);
  ...
  const [openOwnerShipHistoryModal, setOpenOwnerShipHistoryModal] =
    useState<boolean>(false);
  ...
  useEffect(() => {
    const getEditions = async () => {
      ...
      // get song list
      const response = await musicDaoGetEditionsOfSong(id);
      if (response.success) {
  ...
  useEffect(() => {
    ...
    if (songDetailData?.imageUrl) {
      setSongImageIPFS(songDetailData?.imageUrl);
    } else {
  ...
  // const getAvailableAmount = useMemo(() => {
  //   if (isOwner) {
  //     const item = songDetailData?.ownerAddresses.find(
  //       (item) => item.ownerAddress == account?.toLowerCase()
  //     );
  //     return item?.amount || 0;
  //   } else {
  //     const item = songDetailData?.ownerAddresses.find(
  //       (item) => item.ownerAddress == songDetailData?.sellingOffer?.Beneficiary
  //     );
  //     return item?.amount || 0;
  //   }
  // }, [isOwner, songDetailData, account]);
        ...
        return [
          {
            ...
            cell: (
              <Box
                ...
                <Box className={classes.typo1} style={{ fontWeight: 700 }}>{type}</Box>
              </Box>
            )
          },
          {
            ...
            cell: (
              ...
              >
                <img
                  ...
                  alt="avatar"
                />
                <Box className={classes.typo2}>0xeec9...82f8</Box>
              </Box>
            )
          },
          {
  ...
  const handleGotoScan = () => {
    ...
    window.open(
      `https://${!isProd ? 'mumbai.' : ''}polygonscan.com/token/${address}?a=${songDetailData.tokenId
      }`,
      '_blank'
    );
  };
  ...
  const handleOpenIpfs = () => {
    window.open(songDetailData.uri, '_blank');
  };
  ...
  const handleOpensea = () => {
    ...
    const address = songDetailData.podAddress
      ...
      : songDetailData.podInfo.id;
    window.open(
      `https://${!isProd ? 'testnets.' : ''}opensea.io/assets/${!isProd ? 'mumbai' : 'matic'
  ...
  const onClickAvatar = (type) => {
    if (type == 'owner') {
      if (songDetailData.owner.userId || songDetailData.owner.urlSlug) {
        history.push(
          `/profile/${songDetailData.owner.urlSlug ?? songDetailData.owner.userId
          }`
        );
      ...
      if (songDetailData.creatorId || songDetailData.creator?.urlSlug) {
        history.push(
          `/profile/${songDetailData.creator?.urlSlug ?? songDetailData.creatorId
          }`
        );
              ...
              <Box className={classes.flexBox}>
                <Box className={classes.svgBox}>
                  <div ref={anchorShareMenuRef} onClick={showShareMenu}>
                    <ShareIcon />
                    {/* <img src={require("assets/icons/share_dark.webp")} alt="share" onClick={showShareMenu} /> */}
                  </div>
                </Box>
                {openShareMenu && (
                  <Popper
                ...
                {/* <Box className={classes.fruitSection}>{`${
                  (songDetailData &&
                    songDetailData.fruits &&
                    songDetailData.fruits?.length) ??
                  0
                } Fruits`}</Box>
                {isSignedin && (
                  <Box ml={2}>
              ...
              <Box display="flex" justifyContent="space-between" mt={7}>
                ...
                <Box flex={1} ml={5} style={{ width: 'calc(100% - 450px)' }}>
                  ...
                  <Box display="flex" alignItems="center" mt={1}>
                    ...
                    >
                      <PlayIcon fill="white" />
                      <span>{`${songDetailData?.listenedCount ?? 'No'
                        } Plays`}</span>
                    </Box>
                    ...
                    >
                      <ViewIcon fill="white" />
                      <span>{`${songDetailData?.viewCount ?? 'No'
                        } Views`}</span>
                    </Box>
                  </Box>
