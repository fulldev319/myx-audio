...
export default function ProposalCard({ item }) {
  ...
  return (
    <div
      ...
      onClick={() => history.push(`/governance/proposals/${item.id}`)}
    >
      <div className={classes.content}>
        ...
        <Box className={classes.nameSection}>
          ...
          <Box ml={2}>
            <Box className={classes.name}>{item.name || item.urlSlug}</Box>
          </Box>
        </Box>
            ...
            {item.votes && item.totalVotes && (
              <>
                <BorderLinearProgress
                  ...
                  value={
                    Math.round(
                      ((item.votes?.filter((vote) => vote.answer).length || 0) /
                        item.totalVotes) *
                        100 *
                        100
                    ) / 100
            ...
            {item.votes && item.totalVotes && (
              <>
                <BorderLinearProgress
                  ...
                  value={
                    Math.round(
                      ((item.votes?.filter((vote) => !vote.answer).length ||
                        0) /
                        item.totalVotes) *
                        100 *
                        100
          ...
          <div>
            ...
            <Box display="flex" justifyContent="center">
              <Box display="flex" flexDirection="row">
                <span
                  style={{ color: '#65CB63', fontSize: 14, fontWeight: 500 }}
                >
