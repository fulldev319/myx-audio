...
export default function SignInModal(props) {
  ...
  return (
    <Modal isOpen={props.open} onClose={handleClose} size="small">
      <div>
        <img
          src={require('assets/icons/x_darkblue.webp')}
          alt={'x'}
        ...
        {type === 'signIn' && (
          <>
            ...
            <ModalButton
              ...
              variant="outlined"
              onClick={() => setType('signInEmail')}
            >
              Sign In with Email
            </ModalButton>
            <ModalButton
              className={classes.button}
              onClick={() => setType('signInWallet')}
            >
              Sign In with Wallet
            </ModalButton>
          </>
        ...
        {(type === 'signIn' || type === 'signInEmail') && (
          <>
            <div>
              ...
              <div
                className={classes.linkButton}
                onClick={showResendEmailValidation}
              >
        ...
        {type === 'signUp' && (
          <SignUp handleGoSignIn={() => setType('signIn')} setType={setType} />
        )}
        {type === 'signUpWithEmail' && (
          <SingUpWithEmail handleGoSignIn={() => setType('signIn')} />
        )}
        {type === 'signUpWithWallet' && (
          <SingUpWithWallet handleCloseModal={handleClose} />
        )}
        {type === 'forgotPassword' && (
          <ForgotPassword handleGoSignIn={() => setType('signIn')} />
        )}
