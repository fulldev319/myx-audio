...
export const rcpEndpoint =
  process.env.REACT_APP_WAX_NODE_URL || 'https://jungle3.cryptolions.io';

const testWaxUsername = process.env.REACT_APP_TEST_WAX_USERNAME;
const testWaxActivePubKey = process.env.REACT_APP_TEST_WAX_ACTIVE_PUBKEY;
const testWaxOwnerPubKey = process.env.REACT_APP_TEST_WAX_OWNER_PUBKEY;
...
export const getWaxInstance = (username?: string, pubKeys?: string[]) => {
  ...
  return new WaxJS.WaxJS(
    rcpEndpoint,
    account,
    pubkeys.filter((key) => !key).length === 0
      ? (pubkeys as string[])
      : undefined,
    true
  );
        ...
        actions: [
          {
            ...
            data: {
              from,
              to: process.env.REACT_APP_MUSIC_WAX_ACCOUNT,
              assetids: [assetId],
              memo: 'Transfer to Music'
            }
          }
        ]
      },
...
export const getWaxBalances = async (
  wax: WaxJS.WaxJS,
  username: string
): Promise<WaxWalletBalance[]> => {
  try {
    const results = await (wax as any).rpc.get_table_rows({
      json: true, // Get the response as json
