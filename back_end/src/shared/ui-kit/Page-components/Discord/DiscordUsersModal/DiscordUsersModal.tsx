...
const DiscordUsersModal = (props: any) => {
  ...
  const [isPossibleUsersLoading, setIsPossibleUsersLoading] =
      ...
      .then((response) => {
        if (response.data.success) {
          ...
          data.forEach((possibleUsr, i) => {
            let usrsIndex = usrs.findIndex(
              (usr) => usr.userId === possibleUsr.userId
            );
            if (usrsIndex !== -1) {
              data.splice(i, 1);
            }
    ...
    axios
      ...
      .then((response) => {
        if (response.data.success) {
          ...
          const index = possibleUsrs.findIndex(
  ...
  const UserRow = (propsFunction: any) => {
    return (
      <div className="userRowDiscordUsers">
        <div className="userLabelDiscordUsers">
          {propsFunction.user.userName}
        </div>
            ...
            <div className="tableUsersDiscordUsers">
              {possibleUsers && possibleUsers.length > 0
                ? possibleUsers.map((user, i) => (
                    <UserRow user={user} add={true} key={i} />
                  ))
                : null}
            </div>
          </LoadingWrapper>
          ...
          <LoadingWrapper loading={isUsersLoading}>
            <div className="tableUsersDiscordUsers">
              {users && users.length > 0
                ? users.map((user, i) => (
                    <UserRow user={user} add={false} key={i} />
                  ))
                : null}
            </div>
          </LoadingWrapper>
