...
export const ChooseWalletModal: React.FunctionComponent<
  ChooseWalletModalProps
> = ({ isOpen, onClose, onAccept, theme }) => {
                ...
                {walletList.map((wallet, index) => (
                  ...
                  >
                    <div className={styles.walletInfo}>
                      ...
                      <p>Buy with your Music Wallet</p>
                    </div>
                    <div className={styles.walletIcon}>
                      ...
                      wallet.name.toUpperCase().includes('METAMASK') ? (
                        ...
                        wallet.walletType.toUpperCase().includes('WAX') ? (
                        <img
                          src={require('assets/walletImages/waxWallet.webp')}
                          width={35}
                          alt="wax wallet"
                        />
                      ) : (
...
const WalletTile = styled.div<WalletTileProps>`
  background: ${(p) =>
    `${
      p.theme && p.theme === 'dark' ? 'rgba(255, 255, 255, 0.16)' : '#ffffff'
    }`};
  box-shadow: ${(p) =>
    `${!p.theme ? ' 0px 2px 12px rgba(0, 0, 0, 0.1)' : 'none'}`};
  border-radius: ${(p) => (!p.theme ? '20px' : '0')};
  font-family: ${(p) =>
    `${p.theme && p.theme === 'dark' ? 'Agrandir GrandLight' : 'Agrandir'}`};
  display: flex;
