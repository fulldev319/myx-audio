...
const exchange = (network) => {
  const contractAddress =
    config[network].CONTRACT_ADDRESSES.ERC20_TOKEN_EXCHANGE;
  const metadata = require('shared/connectors/web3/contracts/ERC20TokenExchange.json');

  const CreateERC20TokenExchange = async (
    web3: Web3,
    account: string,
    exchangeName: string,
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const retData = {
          ...
          initialOfferId:
            response.events.ERC20TokenExchangeCreated.returnValues[1]
        };
  ...
  const PlaceERC20TokenBuyingOffer = async (
    web3: Web3,
    account: string,
    exchangeId: string,
    amount: string,
    ...
    return new Promise(async (resolve) => {
      try {
        const payload = {
          ...
          price: await web3.utils.toWei(price, erc20ToWeiUnit[token] ?? 'ether')
        };
        const contract = ContractInstance(web3, metadata.abi, contractAddress);
        const gas = await contract.methods
        ...
        resolve({
          success: true,
          data: response.events.ERC20TokenBuyingOfferPlaced.returnValues[0]
        });
      } catch (e) {
        console.log(e);
        resolve({ success: false });
  ...
  const PlaceERC20TokenSellingOffer = async (
    web3: Web3,
    account: string,
    exchangeId: string,
    amount: string,
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        resolve({
          success: true,
          data: response.events.ERC20TokenSellingOfferPlaced.returnValues[0]
        });
      } catch (e) {
        console.log(e);
        resolve({ success: false });
  ...
  const BuyERC20TokenFromOffer = async (
    web3: Web3,
    account: string,
    exchangeId: string,
    offerId: string
  ...
  const SellERC20TokenFromOffer = async (
    web3: Web3,
    account: string,
    exchangeId: string,
    offerId: string
